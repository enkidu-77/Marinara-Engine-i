...
| `PROVIDER_LOCAL_URLS_ENABLED`    | `false`                                                  | Allows user-configured LLM/provider test/model URLs to call local/private addresses. Needed for remote-hosted access to self-hosted provider endpoints.                                                                                                                                                                                                                                       |
...
Outbound provider, image, TTS, DeepLX, and webhook requests reject loopback/private/metadata destinations by default to prevent SSRF. Enable only the feature-specific switch you need for self-hosted services, such as `PROVIDER_LOCAL_URLS_ENABLED=true` for a local OpenAI-compatible endpoint or `IMAGE_LOCAL_URLS_ENABLED=true` for ComfyUI/SD Web UI.
